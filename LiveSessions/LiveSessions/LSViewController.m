...
- (void) startVideoChat:(id) sender
{
    ...
    m_receiverID = [receiverID copy];
    [self goToStreamingVC];
    [PFPush sendPushMessageToChannelInBackground:@"global" withMessage:@"Incoming Call" ];
}
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
    {
        if (!error)
        {
            for (PFObject *object in objects)
            {
                ...
                NSString *userStarRating = [object valueForKey:@"userStarRating"];
           ...
           fromLocation:(CLLocation *)oldLocation
{
    ...
    NSLog(@"## AppDelegate Latitude  : %f",appDelegate.currentLocation.coordinate.latitude);
